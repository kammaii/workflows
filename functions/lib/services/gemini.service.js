...
class GeminiService {
    constructor() {
        this.ai = new generative_ai_1.GoogleGenerativeAI(config_1.config.ai.geminiKey);
    }
    async generateKoreanDraft(title, body) {
...
`;
    ...
    async generateEnglishFinal(title, content) {
        const model = this.ai.getGenerativeModel({
            model: 'gemini-2.5-pro',
            generationConfig: {
                responseMimeType: 'application/json',
...
# Output Format (JSON ONLY)
{
  ...
  "rank_math_permalink": "english-slug"
    ...
    async generateThreads(title, content, link) {
        const model = this.ai.getGenerativeModel({
            model: 'gemini-2.5-pro',
            generationConfig: {
                responseMimeType: 'application/json',
